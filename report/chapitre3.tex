...
\paragraph{Le processus d'analyse de données} \label{sec:process-data-analysis}
...
Pour précision, ces étapes peuvent s'appliquer dans un contexte où les données très volumineuses  mais aussi dans le cas des données manipulées par des outils traditionnels.


IBM Knowledge Center \footnote{Source : \url{https://www.ibm.com/support/knowledgecenter/fr/SSEPGG_9.5.0/com.ibm.im.easy.doc/c_dm_process.html}, consultée le $06/08/2018$.} a résumé le processus de l'analyse de données dans les étapes suivantes :



\subparagraph{Définition de problème.} c'est le point d'entrée vers tout projet d'analyse de données. Les objectifs doivent être clairs. A la fin de cette étape, on sait les objectifs d l'analyse, mais pas forcément comment 
cette analyse va être faite (par exemple, les technologies et les méthodes statistiques).

\subparagraph{Exploration de données. } ce que caractérise cette étape, c'est la découverte de la nature  des données, les sources des données, la qualité des données, etc. 

\subparagraph{Préparation de données} \label{par:step-preparedata}: c'est durant cette phase que se passent les opérations d'ETL présentés dans la section \ref{sec:etl}, ce qui inclut le chargement, le nettoyage et la transformation des données. En parallèle, des tentatives sont faites pour créer le schéma des données avec la possibilité de revoir des sources complémentaires de données ou bien de nouvelles transformations des données. Le schéma comporte les différentes tables, les attributs pertinents et autres.

\subparagraph{Modélisation} : à l'issue de cette phase, un modèle de qualité est créé. Un modèle qui devrait  répondre aux objectifs précédemment établis. Cette étape implique des principes en statistiques, probabilités, \textit{machine learning} ou bien autres fondements.

\subparagraph{Evaluation} : c'est à cette étape qu'on valide le modèle créé à l'étape de  Modélisation. L'évaluation et la validation du modèle prend en compte les objectifs prédéfinis; si le modèle répond aux attentes.

\subparagraph{Déploiement} : c'est l'utilisation  des outils existants ou  la mise place d'une solution qui convient aux  besoins spécifiques en terme de visualisation des résultats de l'analyse.
...
Spark dispose d'une abstraction notée Resilient Distributed Datasets (ou RDDs). Un RDD est une collection d'objets immuable. Ces objets sont répartis sur les n\oe{}uds du cluster de sorte à être traités en parallèle. Un RDD peut être persisté dans la mémoire pour éventuelle réutilisation.   D'ailleurs de cette façon, on constate l'amélioration des performances. En outre, un RDD peut être persisté sur un disque. 
...
input data sont les données à analyser en utilisant Spark. Ces données sont récupérées depuis des sources extérieures vers Spak. Ce dernier crée un RDD basé sur ces données. Un RDD est représenté par le rectangle en orange, les morceaux en orange dans le rectangle représentent les partitions d'un RDD. 
Il existe plusieurs transformations à appliquer sur un RDD, avec la possibilité d'enchaîner plusieurs transformations. Comme une transformation est à la base  \textit{lazy}, les partitions sont partagées sur les n\oe{}uds du cluster qu'à suite à l'appel d'une action. Une fois une partition est localisé sur un n\oe{}ud donné, les transformations ainsi que les actions peut s'enchaîner.
