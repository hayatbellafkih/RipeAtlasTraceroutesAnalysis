...
object AnalyseTraceroutes {
   ...
   */
  def findMedianFromSignals(hop: Hop): ProceedHop = {
   ...
   */
  def findAllLinks(firstRouter: ProceedHop, nextRouter: ProceedHop): Seq[Link] = {
    var links = Seq[Link]()
  ...
  def main(startTimeWindow: DateTime, endTimeWindow: DateTime): Unit = {
    ...
    val rangeDates = generateDateSample(1514769809, 1514797809, 3600)
    val timewindow = 3600
