...
\subparagraph{Notations} Un traceroute  est un ensemble de sauts $h_i$ auxquels sont joints l'identifiant de la sonde ayant effectué la requête traceroute,  la destination de la requête, le temps de la requête, etc. Chaque saut contient un ensemble de signaux. Chaque signal se caractérise par le routeur ayant émis une réponse à la sonde parmi les routeurs traversés avant d'atteindre la destination finale et le RTT correspondant. 
Soit  $h_i$ le i\up{ème} saut d'un traceroute donné tel que

 $h_i =\{s_{i,j} |  j \in [1, S], i\in [1,H]  \}$, $S$ et $H$ sont des entiers. Soit $ s_{i,j} $  le j\up{ème} signal du i\up{ème} saut, il se caractérise par le routeur  source du signal $from_{i,j}$ et le RTT correspondant  $rtt_{i,j}$. C'est ce qu'illustre  la Figure \ref{fig:traceroute}.
 %Chaque saut est décrit par un ensemble de signaux dans  $S$.  
 %Chaque signal décrit le routeur   
 ...
 %,  j\in [1,3]$ dont le routeur émettant le signal est $from_{i,j}$ et son RTT est égal à $rtt_{i,j}$. 
...
\end{wrapfigure} 
%------------------------------------------
%Par définition, la médiane d'un ensemble est la valeur qui permet de diviser cet ensemble en deux parties égales.
...
\subsubsection{Méthode du score de  Wilson}
...
%Tandis que pour l'approche du théorème 2.1,les indices sont obtenus en appliquant 
...
\begin{table}[h]
	...
	\footnotesize{
		...
		\caption{Exemple de Mise à jour de la borne inférieure de l'intervalle  de confiance  de la médiane de référence}
		\label{result-3-cas}
...
\subsection{Détection des anomalies} \label{Detection-anomalies}
...
\begin{figure}[H]
	...
	\captionsetup{justification=centering}
	\resizebox{\textwidth}{!}{
		\input{illustrations/casesAnomalies_2.tex}
	}
	\caption{Illustration des sauts d'un traceroute avec leurs informations}
	\label{fig:anomalies}
\end{figure}
...
\section{Processus de détection des anomalies :  notation par fonctions} \label{processus-de-detection}
